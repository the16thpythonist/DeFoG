...
# the parent of moses_kekrl_b0s43. Reported as a second row so the table shows
# the base and the RL'd model side by side rather than only the tuned one.
#
...
# be more tuning, not less, and the sweep found no separable differences anyway.
# That transfer is disclosed in the manuscript.
#
...
echo "MOSES kekulized E1 -- FROZEN TEST PASS (PRE-RL BASE) @ $(date)"
