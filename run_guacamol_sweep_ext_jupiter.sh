...
#SBATCH --output=guacamol_sweepext_%j.out
...
#
# BOUNDARY CHECK. The main sweep's optimum was eta=50, omega=0.05 -- and eta=50
# was the EDGE of that grid, so the true optimum may lie beyond it. This extends
# to eta in {75, 100} at 500 steps, 12 points.
#
# eta=50 is deliberately RE-MEASURED here rather than reused. It was already
# scored in the main sweep, so the two independent measurements of the same four
# cells give a direct estimate of FCD noise at n=1000 -- which is what decides
# whether the differences between top cells mean anything at all.
#
# Original grid: 32 points (steps x eta x omega) across 4 GPUs, 8 each. Only
...
OUT_DIR="sweep_guacamol_seed42_ext"
...
for i in 0 1 2 3; do
    CUDA_VISIBLE_DEVICES=$i python -u scripts/sweep_sampling.py \
        ...
        --steps 500 --eta 50,75,100 --omega 0,0.05,0.1,0.25 \
        --out-dir "$OUT_DIR" \
        > "guacamol_sweepext_slice${i}_${SLURM_JOB_ID}.out" 2>&1 &
...
echo "grid points completed: ${DONE} / 12"
...
if [ "$DONE" -eq 0 ]; then
    ...
    grep -hA5 "Traceback" guacamol_sweepext_slice*_${SLURM_JOB_ID}.out 2>/dev/null | head -20
    exit 1
fi
