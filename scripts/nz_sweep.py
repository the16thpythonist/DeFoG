...
CELLS = [(1, 12), (10, 12), (50, 10)]          # (n_z, iterations)
BURN = 4
...
print("pre-RL base, coupled, lambda=0.3, lr=3e-4, K=8, minibatch 16\n"
      ...
      f"| {'resid_n':>7s} {'resid_e':>7s} | {'s/it':>6s}", flush=True)
results, evals = {}, []
for n_z, iters in CELLS:
    ...
    for null in (False, True):
        ...
        print(f"  {n_z:4d} {iters:3d} {'null' if null else 'real':5s} | {v['drift']:6.3f} "
              ...
              f"{v['resid_gkl_edges']:7.3f} | {(time.time()-t0)/iters:6.0f}", flush=True)
        if n_z == CELLS[-1][0] and not null:
    ...
    json.dump({"diffs": results, "evals": evals}, open("nz_sweep.json","w"), indent=1)
