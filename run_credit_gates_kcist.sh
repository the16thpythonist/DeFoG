...
HEAD="ckpts/credit/credit_head_${READOUT}_seed${SEED}.ckpt"

echo "host=$(hostname) job=${SLURM_JOB_ID:-none} seed=${SEED} readout=${READOUT} head=${HEAD}"
...
srun "$PY" scripts/gate2_credit.py --head "$HEAD" \
  ...
  --out "ckpts/credit/gate2_${READOUT}_seed${SEED}.json"
...
srun "$PY" scripts/gate3_credit.py --head "$HEAD" \
  ...
  --out "ckpts/credit/gate3_${READOUT}_seed${SEED}.json"
