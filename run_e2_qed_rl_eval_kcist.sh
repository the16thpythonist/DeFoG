...
#SBATCH --job-name=e2_qed_rl
#SBATCH --output=e2_qed_rl_%j.out
...
seen = {}
for ref in ('molsmith/qed@2.0.0', 'molsmith/qed@3.0.0', 'molsmith/qed@3.1.0'):
...
print('  three distinct adapters, all on zinc-kek with heads')
" || exit 1

[ -f "$SIZE_CKPT" ] || { echo "ERROR: ${SIZE_CKPT} missing"; exit 1; }

NAMES=( rl_fk              rl_nofk            rl005_fk           pre_fk )
ADAPT=( molsmith/qed@3.0.0 molsmith/qed@3.0.0 molsmith/qed@3.1.0 molsmith/qed@2.0.0 )
METH=(  fk                 adapter            fk                 fk )
...
for i in 0 1 2 3; do
    if [ "${METH[$i]}" = "fk" ]; then
        CUDA_VISIBLE_DEVICES=$i $PY -u scripts/e2_targeting.py \
            ...
            > "e2qrl_${NAMES[$i]}_${SLURM_JOB_ID}.out" 2>&1 &
    else
        CUDA_VISIBLE_DEVICES=$i $PY -u scripts/e2_targeting.py \
            --adapter "${ADAPT[$i]}" --property qed --split validation \
...
if [ "$OK" -lt 4 ]; then
    ...
    grep -hA6 "Traceback" e2qrl_*_${SLURM_JOB_ID}.out 2>/dev/null \
...
PRIOR = {"pre_fk": 0.0865}  # job 43071, identical config
rows = [("pre_fk",   "pre-RL  + size + FK1000"),
        ("rl_nofk",  "RL-k010 + size          "),
        ("rl005_fk", "RL-k005 + size + FK1000 "),
        ("rl_fk",    "RL-k010 + size + FK1000 ")]
...
if "pre_fk" in got:
    v = got["pre_fk"]["mae_pooled"]; delta = abs(v - PRIOR["pre_fk"])
    print(f"  pre-RL+size+FK1000 here {v:.4f} vs job 43071 {PRIOR['pre_fk']:.4f}  "
          f"|diff| {delta:.4f}  " + ("OK" if delta < 0.003 else
