...
#SBATCH --job-name=moses_base_score
#SBATCH --output=moses_base_score_%j.out

# Score the FROZEN MOSES test pass for the PRE-RL BASE (moses_kek_seed44),
# the second row of the E1 table, with the official molsets suite.
#
# Same references, same implementation, same command shape as the RL model's
# scoring (job 42993) -- the two rows must differ only in which checkpoint
# generated the SMILES, or the comparison between them means nothing.
#
