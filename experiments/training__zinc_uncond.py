"""
Unconditional DeFoG training on ZINC 250k, for a single-node KCIST LR sweep.

Settled ZINC recipe (batch 24, 20 epochs, 9L/256, RRWP-20, marginal noise,
molecular features, EMA, seed 42) but UNCONDITIONAL (cond_dim=0, no data.y).

End-of-run evaluation on NUM_EVAL_SAMPLES=2500 molecules generated in chunks
(EVAL_CHUNK, never one batch):
  - NUV = validity x uniqueness x novelty,
  - distributional match to the dataset on 4 cheap properties (logP, QED, SAS,
    TPSA) via KL divergence: per-property KL, the average KL, and the GuacaMol
    normalized score = mean_i exp(-KL_i) in (0,1] (1.0 = perfect match),
  - one artifact per property (+ a 2x2 overview): the dataset distribution as a
    gray background with the sampled distribution overlaid in the foreground.

Usage:
    python experiments/training__zinc_uncond.py
    python experiments/training__zinc_uncond.py --LEARNING_RATE 3e-4
    python experiments/training__zinc_uncond.py --__TESTING__ True
"""
import os
import sys
import random
...
# --- Training (settled ZINC recipe: batch 24 -> ~10.4k steps/epoch on 250k) ---
EPOCHS: int = 20
BATCH_SIZE: int = 24
LEARNING_RATE: float = 4e-4          # swept per-arm (2e-4/3e-4/4e-4/5e-4)
...
# :param NUM_EVAL_SAMPLES / EVAL_CHUNK:
#     2500 molecules for well-resolved distributions, generated EVAL_CHUNK at a
#     time (never a single 2500 batch).
NUM_EVAL_SAMPLES: int = 2500
EVAL_CHUNK: int = 50

# :param REF_SUBSAMPLE / KL_BINS:
#     Dataset reference property distributions are computed on REF_SUBSAMPLE
#     molecules (all 4 props via the same callbacks as generated, for
#     consistency). KL is computed GuacaMol-style via Gaussian KDE (near-zero
#     finite-sample floor: ~0.001 for an identical distribution); KL_BINS only
#     sets the histogram resolution of the plotted distributions.
...
# Cheap property callbacks used for both the dataset reference and generated mols.
PROPERTY_FNS: dict = {
...
def compute_props(smiles_iter, fns):
    """Compute each property over an iterable of SMILES; skip failures."""
    out = {p: [] for p in fns}
...
def property_kl_and_bins(ref, gen, n_bins_plot):
    ...
    dataset's 0.5-99.5 percentile range, plus histogram bin edges for plotting.
    KDE removes the histogram finite-sample floor (~0 for identical dists).
    Returns (kl, bins); kl is NaN if the generated set is too small/degenerate."""
...
def experiment(e: Experiment) -> None:
    e.log("ZINC 250k UNCONDITIONAL training")
    ...
    fig.suptitle(f"ZINC uncond LR={e.LEARNING_RATE}  |  NUV={nuv:.3f}  "
...
def testing(e: Experiment):
    ...
    smoke = os.path.join(folder_path(__file__), "_zinc_uncond_smoke.csv")
