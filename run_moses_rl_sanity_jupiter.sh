...
#SBATCH --output=moses_rl_%j.out

# JUPITER: GDPO sanity RL on the four ZINC E1 bases, one seed per GPU.
#
# Baseline from the MOSES E1 test pass (4 seeds, n=25,000):
#   validity 0.8866 | disconnected 0.0057 | wonky rings 0.0022 | sanity 0.8846
#
# NOTE this is NOT a ZINC-like case. MOSES barely has the disconnection or ring
# problems -- 0.57% and 0.22%, both near zero. Essentially all of its 11.5%
# headroom is INVALIDITY. So this is effectively a validity run: the graded
# reward handles it correctly (invalid scores 0, so the gradient concentrates
# where the failures are), but do not expect movement on disconnected or rings
# because there is almost nothing there to fix.
#
# Largest headroom of the three datasets, so potentially the biggest gain.
#
...
echo "MOSES GDPO sanity RL @ $(date)"
...
for i in 0 1 2 3; do
    ...
    CUDA_VISIBLE_DEVICES=$i python -u experiments/gdpo_sanity.py \
        ...
        --SEED ${s} \
        --EVAL_ETA 25.0 \
        --BASE_CKPT "'ckpts/moses_e1_seed${s}/best_model'" \
        --OUT_CKPT_DIR "'ckpts/moses_rl_seed${s}'" \
        --__DEBUG__ False \
        > "moses_rl_seed${s}_${SLURM_JOB_ID}.out" 2>&1 &
...
OK=0
for s in "${SEEDS[@]}"; do
    grep -qE "saved final-iteration model|saved RL model" "moses_rl_seed${s}_${SLURM_JOB_ID}.out" 2>/dev/null && OK=$((OK+1))
done
echo "arms that produced a model: ${OK} / 4"
if [ "$OK" -lt 4 ]; then
    echo "ERROR: not all arms completed; tracebacks follow"
    grep -hA5 "Traceback" moses_rl_seed*_${SLURM_JOB_ID}.out 2>/dev/null | head -20
    exit 1
fi
for s in "${SEEDS[@]}"; do
    echo "--- seed ${s} ---"
    grep -E "BEFORE|validity \(relaxed\)|disconnected|wonky rings|sanity \(all" \
        "moses_rl_seed${s}_${SLURM_JOB_ID}.out" | tail -6
done
